...
class FileService:
    ...
    def open_todo_folder(self, folder_path: str) -> tuple[bool, str]:
        ...
        Args:
            folder_path: 열 폴더의 경로 (상대 경로 또는 절대 경로)
        ...
        if not os.path.isabs(folder_path):
            # 상대 경로인 경우 base_folder_path와 결합
            absolute_path = os.path.join(self.base_folder_path, folder_path)
        else:
            absolute_path = folder_path
        
        # 경로 정규화 (백슬래시/슬래시 통일, .. 처리 등)
        ...
        if not os.path.exists(absolute_path):
            try:
                ...
                print(f"폴더가 존재하지 않아 새로 생성했습니다: {absolute_path}")
            except OSError as e:
        ...
        try:
            ...
            if system == "Windows":
                # Windows에서는 여러 방법 시도
                try:
                    # 방법 1: os.startfile 사용 (Windows 전용, 가장 안정적)
                    os.startfile(absolute_path)
                    return True, ""
                except Exception as e1:
                    try:
                        # 방법 2: explorer 명령어 사용 (경로를 따옴표로 감싸기)
                        subprocess.run(f'explorer "{absolute_path}"', shell=True, check=True, timeout=10)
                        return True, ""
                        ...
                        try:
                            ...
                            return False, f"Windows 탐색기를 실행할 수 없습니다.\n시도한 방법들이 모두 실패했습니다:\n1. os.startfile: {e1}\n2. explorer 명령어: {e2}\n3. subprocess.Popen: {e3}\n\n수동으로 다음 경로를 열어주세요:\n{absolute_path}"
                            
            elif system == "Darwin":  # macOS
                # macOS에서는 open 사용
                subprocess.run(["open", absolute_path], check=True, timeout=10)
                return True, ""
                
            elif system == "Linux":
                # Linux에서는 xdg-open 사용
                subprocess.run(["xdg-open", absolute_path], check=True, timeout=10)
                return True, ""
            ...
            return False, f"폴더 열기 시간이 초과되었습니다. 시스템이 응답하지 않을 수 있습니다.\n경로: {absolute_path}"
            ...
            if system == "Darwin":
                ...
                return False, f"예상치 못한 오류가 발생했습니다.\n오류 코드: {e.returncode}\n수동으로 다음 경로를 열어주세요:\n{absolute_path}"
        except Exception as e:
            return False, f"폴더 열기 중 예상치 못한 오류가 발생했습니다:\n{e}\n\n수동으로 다음 경로를 열어주세요:\n{absolute_path}"
